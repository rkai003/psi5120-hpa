...
cluster gerenciado em Amazon EKS. A mesma imagem de contêiner, os mesmos
...
cinco segundos e registrou uma série temporal, o que permitiu medir o tempo de
reação em vez de apenas constatá-lo. Os resultados mostram latência de detecção
...
A elasticidade é frequentemente apresentada como característica definidora da
computação em nuvem, entendida como a capacidade de ajustar recursos à demanda
sem intervenção manual. Em plataformas de orquestração de contêineres, essa
propriedade se materializa em mecanismos concretos, e o Horizontal Pod
Autoscaler é o mais direto entre eles.
...
O denominador dessa razão merece atenção porque condiciona todo o mecanismo. A
utilização não é medida em valor absoluto, e sim como proporção do recurso
...
A definição desse consumo envolve uma decisão metodológica relevante. A carga é
especificada por \emph{tempo de processador} e não por número de iterações. Se
fosse definida por iterações, um processador mais rápido concluiria o trabalho
em menos tempo e consumiria menos CPU-segundos por requisição. Como os dois
ambientes comparados possuem processadores distintos, parte da diferença
observada no autoescalamento decorreria do equipamento e não do controlador,
o que constituiria variável de confusão. Ao fixar o tempo de processador, cada
requisição custa aproximadamente os mesmos CPU-segundos nos dois ambientes.
...
milissegundos, o que corresponde a erro de 0,17 por cento.
...
Duas diferenças merecem destaque por seus efeitos sobre os resultados. Os
...
O tipo de instância adotado no ambiente gerenciado foi \texttt{c7i-flex.large}.
A escolha decorre de restrição da conta utilizada, que opera em plano gratuito e
admite somente tipos elegíveis. Tentativa anterior com \texttt{t3.medium},
tipo de capacidade equivalente, resultou em falha de provisionamento com a
mensagem \texttt{InvalidParameterCombination}. A diferença entre os dois tipos
é de elegibilidade comercial e não de capacidade técnica, o que ilustra como
restrições contratuais podem condicionar decisões de arquitetura.
...
\subsection{Detecção automática dos eventos}

O instante de início da carga é determinado durante a análise como o primeiro
momento em que a utilização observada ultrapassa o alvo configurado, e não por
anotação manual do horário de aplicação do gerador. O critério é idêntico nos
dois ambientes e elimina o erro humano de registro, que poderia ser da mesma
ordem de grandeza das diferenças que se pretende medir.
...
\subsection{Políticas de escalamento}

As políticas de comportamento do autoescalador foram declaradas explicitamente
no manifesto, ainda que sejam opcionais. A razão é metodológica, pois os dois
clusters executam versões distintas do Kubernetes e valores padrão podem
divergir entre versões, o que introduziria variável não controlada.

Para aumento, a janela de estabilização foi definida como nula e a política
autoriza o maior valor entre dobrar a quantidade de réplicas e acrescentar
quatro réplicas a cada quinze segundos. Para redução, foi mantida a janela
padrão de trezentos segundos.

A assimetria entre as duas direções não constitui deficiência do controlador.
Aumentar tardiamente degrada o serviço, ao passo que reduzir precocemente
provoca oscilação de réplicas diante de variações transitórias de carga.
...
foi de 16 segundos no ambiente local e 14 segundos no gerenciado. A proximidade
era esperada, uma vez que ambos dependem do mesmo ciclo de reconciliação de
quinze segundos e do mesmo intervalo de coleta do Metrics Server. Os valores
correspondem, na prática, a um único ciclo de controle, o que representa a
menor latência possível para o mecanismo.
...
A razão entre os dois valores é próxima de dois.

Como as políticas de escalamento foram declaradas de forma idêntica e a latência
de detecção é equivalente, a diferença não decorre da decisão do controlador, e
...
A observação tem consequência prática. Testes de autoescalamento conduzidos em
...
Esse resultado cumpre função de verificação. Um parâmetro de valor conhecido foi
medido de forma independente, em dois ambientes distintos, e reproduziu o valor
esperado com desvio de 1 por cento. A concordância sustenta a confiabilidade das
demais grandezas obtidas pela mesma instrumentação.

A magnitude do intervalo também é informativa. A redução leva aproximadamente
...
Duas decisões de projeto reduziram o custo total. O modelo de rede adotado
emprega apenas sub-redes públicas, o que dispensa a criação de \emph{NAT
Gateway}, recurso cobrado por hora e por volume processado. Os registros do
plano de controle foram mantidos desabilitados, evitando cobrança por ingestão
e retenção. Ambas as escolhas reduzem, na mesma medida, a capacidade de
